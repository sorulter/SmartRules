...
current:
	...
	@go build -ldflags "-s -w -X main.Git=$(GIT)" -o ./bin/pac ./src/*.go

linux:
	@printf "\e[34m Build Linux Binary \e[0m\r\n"
	@GOARCH=amd64 GOOS=linux go build -ldflags "-s -w -X main.Git=$(GIT)" -o ./bin/pac-64x-linux ./src/*.go

run: dependencies current
	@ls -l ./bin/*
	@printf "\e[32;47m RUN Server \e[0m"
	./bin/pac -logtostderr #-log_dir="logs/"
